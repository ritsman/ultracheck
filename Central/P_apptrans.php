...
if($dtd==""){
    ...
    $q="select * from `Q__challan_main` where fromm='$outlet-UC'";
}else{
    $q="select * from `Q__challan_main` where fromm='$outlet-UC'  and (chdt between '$dtd' and '$dtd2')";
...
$line_head.="<th>TO</th><th>PCS</th><th>APproVe</th></tr>";
echo $line_head;
//return false;
$tpc=0;
foreach($data as $d){
    ...
    $line.="<td>$d[fromm]</td>";
    $line.="<td>$d[too]</td>";
    $line.="<td><a href='#' id='$d[pono]' class='$d[too]' onclick=\"open_chln(this)\">$d[pcs]</a></td>";
    $line.="<td>$d[balpcs]</td>";
...
function open_chln(ele){
    var id=$(ele).attr("id");
    var desti_outlet=$(ele).attr("class");
    console.log(id,outlet);
    $("#holder1").val(id);
    $("#holder2").val(outlet);
    $("#holder3").val(desti_outlet);
    $("#POForm").submit();
}

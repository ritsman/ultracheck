...
$pono=get_new_pono_rt($DBH,$outlet);

$bill_prefix=get_bill_prefix_rt($DBH,$outlet);
...
function get_challan_rt($DBH,$outlet,$barcode){
    $q="select location from `T__salesdata__$outlet` where barcode='$barcode'";
    $stm=$DBH->prepare($q);
    try {
...
$q="create table if not exists `T__salesmain_rt__$outlet` (";
...
$q2="create table if not exists `T__salesdata_rt__$outlet` (";
...
$q3="insert  into `T__salesmain_rt__$outlet` (";
...
foreach($dataBill as $d){
    ...
    $q4="insert  into `T__salesdata_rt__$outlet` (";
    ...
    var_export($h);
    echo '<hr>';
    $loc=get_challan_rt($DBH,$outlet,$d['barcode']);
    //echo "<br>q4:$q4<br>";
    //var_export($d);
    //echo '<hr>';
    $stm=$DBH->prepare($q4);
    ...
    try {
     ...
     $rtbar='9'.$d['barcode'];
     $rtbar=intval($rtbar);
     $q9="select * from `T__salesdata__$outlet` where barcode='$rtbar'";
     $stm=$DBH->prepare($q9);
     try {
         $stm->execute();
         if($stm->rowCount()>0){
            $rtbar='99'.$d['barcode'];
            $rtbar=intval($rtbar);
            $q6="update  `T__salesdata__$outlet` set barcode='$rtbar' where barcode='$d[barcode]'";
            echo "<br>$q6<br>";
            $stm=$DBH->prepare($q6);
            try {
     ...
     // reinsert into inv
     $inventory_table="Q__stk__$outlet";
     echo '<hr>';
     echo $d['barcode']."--".$inventory_table."--".$billno;
     echo '<hr>';
     $b=new BarcodeCheck($d['barcode'],$inventory_table);
     #####################################
     $c=$b->insert_inventory_rt($billno);
     ##################################
...
$qi="insert into B_po_new_gen_rt(`year`,`month`,`$bill_field`)values('$year','$month','$pono2')";
$stm = $DBH->prepare($qi);
try {
    $stm->execute();

...
foreach($data as $d){
    ...
    if($dataMain['cat']=='TOWEL'||$dataMain['cat']=='NAPKIN'){
       ...
       // <img src="../img88/ultralogo.jpeg" alt="companylogo" style="width:2.5cm;height:2.5cm;margin:0">
        ...
        $pdf->RoundedRect(5,20,40,53,'2','1111','D');

        $pdf->SetFont('','B',9,'','false');

        $pdf->Text(15,22,$dataMain['series'],'','',true,'','','','','','','','','');
        $pro="PRODUCT";
        $catagory=$dataMain['cat'];
        if($dataMain['cat']=='TOWEL'||$dataMain['cat']=='NAPKIN'){
            $pro2="";
            $fit='';
        }else{
        ...
        if($dataMain['series']=='FACE NAPKIN'){
            $pcs=5;
        }else if($dataMain['series']=='NAPKIN HAND'){
            $pcs=2;
        }else if($dataMain['series']=='HANDKERCHIEF'||$dataMain['series']=='HAND NAPKIN'){
            $pcs=3;
        }else {
            $pcs=1+$fit;
        }
        ...
        $pdf->Text(5,35,$pro2,'','',true,'','','','','','','','','');
        $pdf->Text(20,35,$fit,'','',true,'','','','','','','','','');


        $pdf->Text(5,40,$pro3,'','',true,'','','','','','','','','');
        $pdf->Text(20,40,$size,'','',true,'','','','','','','','','');

        //$pdf->Text(5,43,$pro4,'','',true,'','','','','','','','','');
        //$pdf->Text(20,43,$inseam,'','',true,'','','','','','','','','');
       ...
       // $pdf->Text(5,42,$pro5,'','',true,'','','','','','','','','');
        ...
        $pdf->write1DBarcode($d['barcode'], 'C128',4, 80, 38, 8, 0.4, $style, 'N');
        $pdf->Text(10,86,$d['barcode'],'','',true,'','','','','','','','','');
       if($i<$count){
        $pdf->AddPage();
       }
        $i++;

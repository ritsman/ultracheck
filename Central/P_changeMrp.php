...
if($location=='CENTRAL'){
    ...
    $update_query="update `Q__stk__$location` set `mrp`='$mrp' where  status='START'";
}else{
    $q="select * from `Q__stk__$location` where status='CONFM' ";
    $wq="select * from `Q__stk__$location` where status='CONFM' ";
    $update_query="update `Q__stk__$location` set `mrp`='$mrp' where status='CONFM'";
}
//echo $q;
...
//echo $update_query;
//echo "<br>$wq";
//update mrp here and then show the records
$stm22=$DBH->prepare($update_query);
try {
    $stm22->execute();

...
<div class="pageShow2">
    <table id="maintab">
        ...
        <tr>
            <td><input type="text" id="searchbar" title="barcode"/></td>
...
<!-- this is addition for stock verify -->
    <table>
        <tr>
            <td>New MRP</td>
            <td><input type="text" name="sono22" id="sono22" ></td>
        </tr>
        <tr>
            <td>
                <div id="canned"></div>
    ...
    <div id="response"></div>
    <div class="row d-flex justify-content-center m-3 p-2">
    <button type="button" name="updBar" id="updBar" class="btn btn-info m-2">UPDATE</button>
    <button type="button" name="printBar" id="printBar" class="btn btn-info m-2">PRINT</button>
    
    </div>
    

    <form id="POForm" class="noshow" action="reprintbarcode.php" target="_blank" method="post">
    ...
    $("#maintab  tr td").each(function(){
           ...
           if(search_data.goahead=='yes'){
               search_data=JSON.stringify(search_data);
                var di='<div class="loader"></div>';
                $("div#showdata").html(di);
                $("div#showdata").load("P_changeMrp.php",{data:data,search_data:search_data,mrp:mrp},function(response,status,xhr){
                    if(status=='error'){
                        $("#showdata").html("#ERROR in showing data");
                        return false;
                    }
